...
class Database:
    ...
    def use_credit(self, user_id: int, media_type: str) -> bool:
        ...
        if credits.get(daily_key, 0) > 0:
            ...
            self.update_credits(user_id, f"generation_count.{media_type}", 1)
            return True
...
class ForceJoinMiddleware(BaseMiddleware):
    ...
    async def __call__(self, handler, event, data):
        if isinstance(event, types.Message):
            ...
            # Check channel membership for all other messages
            user_id = event.from_user.id
            try:
        ...
        if isinstance(event, types.Message) and event.text:
            ...
            if NSFW_PATTERN.search(event.text):
                ...
                if warnings >= 3:
                    ...
                    await event.answer("❌ You have been permanently blocked for NSFW content!", show_alert=True)
                    return
                else:
                    await event.answer(
                        f"⚠️ NSFW content detected! Warning {warnings}/3\n"
                        "Do not use NSFW prompt or you will be permanently blocked!",
                        show_alert=(warnings == 2)  # Show alert on last warning
                    )
                    return
...
class GeminiGenAPI:
    ...
    async def poll_for_image(self, job_id: str, timeout: int = 300) -> str:
        async with aiohttp.ClientSession(cookies=self.cookies, headers=self.headers) as session:
            ...
            retry_count = 0
            max_retries = 5  # 🔴 CHANGED: Increased from 3 to 5
...
async def cmd_broadcast(msg: types.Message):
    ...
    # Use hash for confirmation but include message ID for uniqueness
...
async def confirm_broadcast(cb: CallbackQuery):
    ...
    for idx, user_id in enumerate(user_ids, 1):
        try:
            # Send message content (skip the header)
...
async def handle_received_photo(msg: types.Message, state: FSMContext, user: Dict):
    ...
    # Check file size (Telegram sends size in file_size attribute)
    ...
    logger.info(f"📐 Detected aspect ratio: {aspect_ratio} ({photo.width}x{photo.height})")
...
async def handle_edit_prompt(msg: types.Message, state: FSMContext, user: Dict):
    ...
    try:
        ...
        logger.info(f"✅ SUCCESS! Sent edited image for job {job_id}")
...
async def handle_new_image_prompt(msg: types.Message, state: FSMContext, user: Dict):
    ...
    try:
        ...
        await status_msg.delete()
        logger.info(f"✅ SUCCESS! Sent new image for job {job_id}")
...
async def handle_video_prompt(msg: types.Message, state: FSMContext, user: Dict):
    ...
    try:
        ...
        await status_msg.delete()
        logger.info(f"✅ SUCCESS! Sent video for job {job_id}")
